...
# Dependency graph for incremental builds. Values mix lists of paths with the
# occasional plain string (output_pattern), so the inner type is deliberately Any.
DEPENDENCIES: Dict[str, Dict[str, Any]] = {
    "music.html": {
        "data": ["data/music.json"],
        "templates": [
            "templates/music-template.html",
    ...
    "writing.html": {
        ...
        "templates": [
            "templates/writing-template.html",
            "templates/header.html",
            "templates/footer.html",
            "templates/favicon.html",
        ],
    ...
    "photos/": {
        ...
        "templates": [
            "templates/photo-page-template.html",
            "templates/photo-gallery-template.html",
            "templates/header.html",
            "templates/footer.html",
            "templates/favicon.html",
        ],
    ...
    "talks/": {
        "data": [
            ...
            "data/current_talks.json",
            "data/pagevariables.json",
        ],
        "templates": [
            "templates/talk-index-template.html",
            "templates/talk-page-template.html",
        ],
        "static": ["public/talks/"],
...
def get_all_dependencies(  # pylint: disable=too-many-nested-blocks
    page_key: str,
) -> Set[str]:
...
def get_latest_modification_time(dependencies: Set[str]) -> float:
    ...
    # float, not int: os.path.getmtime returns a float timestamp
    latest: float = 0
    ...
    def collect_gallery_outputs(directory: str, output_path: str) -> None:
        ...
        for item in items:
            ...
            if os.path.isfile(item_path) and any(
                item.lower().endswith(ext) for ext in [".jpg", ".jpeg", ".gif", ".png"]
            ):
                ...
                # Each image also generates resized versions (not tracked
                # for rebuild logic)

            elif os.path.isdir(item_path):
                # Process subdirectory (gallery generator creates
                # alphanumeric directory names)
...
def get_talks_output_files(  # pylint: disable=too-many-locals,too-many-nested-blocks
            ...
            for talk in conference["talks"]:
                ...
                if get_talk_date(talk) < date.today():
                    ...
                    if talk_type in [
                        "talk",
                        "keynote",
                    ]:  # Only these types get individual pages
                        if talk_index:
...
# re-exported so callers can keep using common.generate_page_metadata; the
# implementation lives in metadata.py to keep this module under the line limit
from kgdotcom.core.metadata import (  # noqa: E402  pylint: disable=wrong-import-position
    PageMetadata,
    generate_page_metadata,
)

__all__ = ["PageMetadata", "generate_page_metadata"]
