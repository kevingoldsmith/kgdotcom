...
for talk_index in unique_talks:
	# TODO: refactor this
	index_page = generate_talk_page(talk_index, unique_talks[talk_index], output_directory,
		index_page, debug_mode)
...
for conference in labs:
	...
	for talk in conference['talks']:
		if conference_talk_types.is_workshop(talk):
			talk_date = common.get_talk_date(talk)
			talk_name = talk['talk']
			index_page['labs'].append({'date': talk_date, 'name': talk_name,
	...
	if ('location' in conference['talks'][0]) and len(conference['conference']) > 0:
		...
		for talk in conference['talks']:
			...
			if 'outputfilename' in talk:
				talks.append(format_a.format(get_talk_url(talk['outputfilename'], debug_mode),
					talk_name))
			else:
...
with open(output_directory+'index-jinja.html', 'w') as file:
