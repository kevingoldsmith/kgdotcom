...
def generate_sitemap(debug_mode=False):
    ...
    for path, _, files in os.walk(path):
        for file in files:
            ...
            if (not file in ignore_files) and (ext == '.html'):
                ...
                sitemap_files.append((os.path.join(path, file),
                    get_href_root(os.path.join(path_list[1], file), debug_mode)))

    sitemap_entries = []
    for file in sitemap_files:
        sitemap_entries.append(format_url.format(url=file[1],
            date=datetime.datetime.fromtimestamp(os.path.getmtime(file[0])).strftime('%Y-%m-%d')))
